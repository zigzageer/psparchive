    ...
    <div className="w-full h-full overflow-y-auto pt-40 pb-20 px-6 custom-scrollbar">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-6 gap-y-12 max-w-[1600px] mx-auto">
        {items.map((item, i) => (
          ...
          >
            <MiniPSP colorHex={item.color.hex} imageUrl={item.color.imageUrl} className="w-32 h-16 group-hover:scale-110 transition-transform" />
            <div className="text-center flex flex-col items-center">
              <div className="text-xs font-bold uppercase tracking-wider leading-tight mb-1.5">{item.color.name}</div>
              ...
              {item.color.type === 'limited' && (
                <div className="text-[9px] font-mono text-amber-400/80 uppercase mt-2 border border-amber-400/30 px-2 py-0.5 rounded-sm bg-amber-400/5">
                  Limited
                </div>
              )}
            </div>
